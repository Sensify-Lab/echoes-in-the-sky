...
def compute_detailed_granger() -> pd.DataFrame:
    ...
    for theme in themes:
        ...
        if len(df) < (MAX_LAG_DAYS * 3) or df["eo"].sum() == 0 or df["bluesky"].std() == 0:
            continue
        eligible_input_rows.append(input_df)
        try:
            results = grangercausalitytests(df[["bluesky", "eo"]], maxlag=MAX_LAG_DAYS, verbose=False)
        except Exception:
            continue
        for lag, res in results.items():
            ...
            rows.append(
                {
                    "relationship": f"{theme}  EO → Bluesky",
                    "theme": theme,
                    "lag_days": lag,
    ...
    if input_rows:
        pd.concat(input_rows, ignore_index=True).to_csv(
            TABLE_DIR / "eo_to_bluesky_granger_input_daily.csv",
            index=False,
        )
    if eligible_input_rows:
        pd.concat(eligible_input_rows, ignore_index=True).to_csv(
            TABLE_DIR / "eo_to_bluesky_granger_input_daily_eligible.csv",
            index=False,
        )
    out = pd.DataFrame(rows).sort_values(["theme", "lag_days"])
    out.to_csv(TABLE_DIR / "eo_to_bluesky_granger_fstats_detailed.csv", index=False)
    return out
...
def plot_fstats(df: pd.DataFrame) -> None:
    ...
    ax.set_title("EO → Bluesky Granger Causality F-statistics (Daily Lags)")
    ...
    stem = "eo_to_bluesky_granger_fstats"
...
def main() -> None:
    ...
    plot_fstats(df)
    print(f"input\t{TABLE_DIR / 'eo_to_bluesky_granger_input_daily.csv'}")
    print(f"eligible_input\t{TABLE_DIR / 'eo_to_bluesky_granger_input_daily_eligible.csv'}")
    print(f"table\t{TABLE_DIR / 'eo_to_bluesky_granger_fstats_detailed.csv'}")
    print(f"figure\t{FIG_DIR / 'eo_to_bluesky_granger_fstats.png'}")
