...
BASE_DIR = Path("local_data/03_Github_data/08_EO_effect_structure_changes/post_2025_01_20")
...
def plot_theme(theme: str, sub: pd.DataFrame, breaks_df: pd.DataFrame) -> None:
    ...
    for _, row in breaks_df.iterrows():
        ...
        ax.axvspan(ci_low, ci_high, color="#d62728", alpha=0.12)
        ax.axvline(brk, color="#7f0000", linewidth=2.2, linestyle="--")
        ax.text(ci_low, label_y, f"CI low\n{ci_low.date()}", color="#7f0000", fontsize=8, ha="left", va="top")
        ax.text(brk, label_y * 0.96, f"Break\n{brk.date()}", color="#7f0000", fontsize=8, ha="center", va="top")
        ax.text(ci_high, label_y, f"CI high\n{ci_high.date()}", color="#7f0000", fontsize=8, ha="right", va="top")
    ...
    stem = slugify(theme)
    fig.savefig(PER_THEME_DIR / f"{stem}_breaks_post_2025_01_20.png", dpi=320, bbox_inches="tight", facecolor="white")
    fig.savefig(PER_THEME_DIR / f"{stem}_breaks_post_2025_01_20.pdf", bbox_inches="tight", facecolor="white")
    plt.close(fig)
...
def main() -> None:
    ...
    summary_df.to_csv(TABLE_DIR / "theme_structure_break_summary_post_2025_01_20.csv", index=False)
    breaks_df.to_csv(TABLE_DIR / "theme_structure_breaks_post_2025_01_20.csv", index=False)

    print(f"daily_full\t{TABLE_DIR / 'theme_daily_posts_full.csv'}")
    print(f"summary\t{TABLE_DIR / 'theme_structure_break_summary_post_2025_01_20.csv'}")
    print(f"breaks\t{TABLE_DIR / 'theme_structure_breaks_post_2025_01_20.csv'}")
