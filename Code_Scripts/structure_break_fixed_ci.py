...
BASE_DIR = Path("local_data/03_Github_data/08_EO_effect_structure_changes/post_2025_01_20_max20_fixed_ci")
...
def breakpoint_date_from_end_index(sub: pd.DataFrame, end_idx_1based: int) -> pd.Timestamp:
    end_idx_1based = max(1, min(int(end_idx_1based), len(sub)))
    return pd.to_datetime(sub["day"].iloc[end_idx_1based - 1])
        ...
        rows.append(
            {
                ...
                "Estimate": bp,
                "CI_97.5": ci_high_idx,
                "CI_2.5_date": breakpoint_date_from_end_index(sub, ci_low_idx),
                "Break_date": breakpoint_date_from_end_index(sub, bp),
                "CI_97.5_date": breakpoint_date_from_end_index(sub, ci_high_idx),
...
def plot_theme(theme: str, sub: pd.DataFrame, breaks_df: pd.DataFrame) -> None:
    ...
    # lightly shade each regime segment
    ...
    segment_colors = ["#f7fbff", "#eef5fb"]
    for i in range(len(segment_bounds) - 1):
        left = segment_bounds[i]
        right = segment_bounds[i + 1]
        ax.axvspan(left, right, color=segment_colors[i % 2], alpha=0.35, zorder=0)
    ...
    for idx, (_, row) in enumerate(breaks_df.iterrows()):
        ...
        label_y_mid = max(label_y - ymax * 0.05, ymax * 0.70)

        ax.axvspan(ci_left, ci_right, color="#d62728", alpha=0.14, zorder=1)
        ax.axvline(ci_low, color="#b22222", linewidth=1.6, linestyle=":", zorder=4)
        ax.axvline(brk, color="#7f0000", linewidth=2.4, linestyle="--", zorder=5)
        ax.axvline(ci_high, color="#b22222", linewidth=1.6, linestyle=":", zorder=4)

        ax.text(ci_low, label_y, f"CI low\n{ci_low.strftime('%b-%Y')}", color="#7f0000", fontsize=8, ha="right", va="top")
        ax.text(brk, label_y_mid, f"Break\n{brk.strftime('%b-%Y')}", color="#7f0000", fontsize=8, ha="center", va="top")
        ax.text(ci_high, label_y, f"CI high\n{ci_high.strftime('%b-%Y')}", color="#7f0000", fontsize=8, ha="left", va="top")
    ...
    stem = slugify(theme)
    fig.savefig(PER_THEME_DIR / f"{stem}_breaks_post_2025_01_20_max20_fixed_ci.png", dpi=320, bbox_inches="tight", facecolor="white")
    fig.savefig(PER_THEME_DIR / f"{stem}_breaks_post_2025_01_20_max20_fixed_ci.pdf", bbox_inches="tight", facecolor="white")
    plt.close(fig)
...
def main() -> None:
    ...
    summary_df.to_csv(TABLE_DIR / "theme_structure_break_summary_post_2025_01_20_max20_fixed_ci.csv", index=False)
    breaks_df.to_csv(TABLE_DIR / "theme_structure_breaks_post_2025_01_20_max20_fixed_ci.csv", index=False)

    print(f"daily_full\t{TABLE_DIR / 'theme_daily_posts_full.csv'}")
    print(f"summary\t{TABLE_DIR / 'theme_structure_break_summary_post_2025_01_20_max20_fixed_ci.csv'}")
    print(f"breaks\t{TABLE_DIR / 'theme_structure_breaks_post_2025_01_20_max20_fixed_ci.csv'}")
